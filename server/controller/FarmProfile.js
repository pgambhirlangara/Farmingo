...
const createFarmProfile = async (req, res) => {
    try {
        ...
        if (farmer) {
            const uploadImage = await upload(`${Date.now() + "" + req.body.farmName}`, req.body.image, 'jpg');
            ...
            return res.status(401).json({
                message: "Farmer Not found, you should be a farmer to create a Farm Profile!"
            })
        }
...
const updateFarmProfile = (req, res) => {
    ...
    FarmProfile.findOneAndUpdate({ _id : id }, req.body)
    .then((result) => {
        res.status(200).json({
